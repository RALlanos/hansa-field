...
export function UniversalMapWorkspace({ catalog }: { catalog: Catalog }) {
  ...
  return (
    <div className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-100 font-sans relative">
      {/* Compact Top Bar */}
      <header className="h-13 bg-slate-900 text-white px-4 flex items-center justify-between border-b border-slate-800 shrink-0 z-[1050]">
      ...
      <div className="flex-1 relative flex">
        {/* Scope Selector Button (when collapsed) */}
        {!isSelectorOpen && (
          <button
            onClick={() => setIsSelectorOpen(true)}
            className="absolute top-3 left-3 z-[1100] flex items-center gap-2 bg-white/95 backdrop-blur-xs border border-slate-300 hover:border-slate-400 text-slate-800 px-3.5 py-2 rounded-lg shadow-xl text-xs font-semibold hover:bg-slate-50 transition cursor-pointer"
            title="Mostrar selector de capas"
          >
            <span className="text-sm">☰</span>
            <span>Capas Activas ({activeLayersCount})</span>
          </button>
        )}

        {/* Scope Selector Floating Drawer / Overlay (in front of the map with z-[1100]) */}
        {isSelectorOpen && (
          <div
            className="absolute top-3 left-3 z-[1100] w-76 bg-white/95 backdrop-blur-xs border border-slate-300 rounded-lg shadow-2xl p-4 space-y-3.5 max-h-[85vh] overflow-y-auto text-xs text-slate-700 pointer-events-auto"
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-700 p-1 rounded hover:bg-slate-100 transition cursor-pointer text-xs"
                title="Minimizar panel de capas"
              >
                ✕
              </button>
            </div>
