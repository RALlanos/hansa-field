...
export function RecordInspector({
  ...
  const [geometryText, setGeometryText] = useState(
    row.geometry ? JSON.stringify(row.geometry, null, 2) : ""
  );
  ...
  const targetCollection =
    catalog.collections.find(
      (c) => c.id === (row.dataset_id || detail?.dataset_id || datasetId)
    ) ?? collection;
  const fields =
    targetCollection?.schema_definition?.sections?.flatMap((s) => s.fields) ?? [];

  // Helper to extract attribute values matching flexible naming
  const getFieldValue = useCallback((vals: Record<string, unknown>, field: any): unknown => {
    if (!vals) return undefined;
    const directKey = (field as any).key;
    if (directKey && vals[directKey] !== undefined) return vals[directKey];
    if (field.id && vals[field.id] !== undefined) return vals[field.id];

    // Check stripped prefixes like f-cod -> cod -> codigo
    const stripped = field.id?.replace(/^f-[a-z0-9]+-/, "").replace(/^f-/, "");
    if (stripped && vals[stripped] !== undefined) return vals[stripped];

    // Try case-insensitive lookup
    const lowerDirect = directKey ? String(directKey).toLowerCase() : "";
    const lowerId = field.id ? String(field.id).toLowerCase() : "";
    const lowerStripped = stripped ? String(stripped).toLowerCase() : "";

    for (const [k, v] of Object.entries(vals)) {
      const lk = k.toLowerCase();
      if (lk === lowerDirect || lk === lowerId || lk === lowerStripped) return v;
    }
    return undefined;
  }, []);

  // Sync state whenever selected row changes
  useEffect(() => {
    if (row.attributes) {
      setValues({ ...row.attributes });
    }
    if (row.geometry) {
      setGeometryText(JSON.stringify(row.geometry, null, 2));
    }
  }, [row.record_id, row.attributes, row.geometry]);
  ...
  const loadDetail = useCallback(async () => {
    ...
    try {
      ...
      if (cached) {
        ...
        setValues((prev) => ({ ...(row.attributes || {}), ...(cached.value.attributes || {}), ...prev }));
        if (cached.value.geometry) {
          setGeometryText(JSON.stringify(cached.value.geometry, null, 2));
        }
      ...
      if (!cached || cached.stale) {
        ...
        if (fresh.geometry) {
          setGeometryText(JSON.stringify(fresh.geometry, null, 2));
        }
      ...
      // Gracefully retain row attributes if backend detail is not cached
      if (row.attributes) {
        setValues((prev) => ({ ...(row.attributes || {}), ...prev }));
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4 space-y-4">
        {activeTab === "attributes" && (
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-600">
                <div>
                  ...
                  <span className="font-mono font-semibold text-slate-800">
                    {String(values.codigo || values.code || row.record_id.slice(0, 10))}
                  </span>
                </div>
                <div>
                  <span className="text-slate-400 block">Ubicación / Zona:</span>
                  <span className="font-medium text-slate-800 truncate block">
                    {String(values.municipio || values.zona || values.departamento || "Bolivia")}
                  </span>
                </div>
              </div>
            </div>
            ...
            {isProjectContext && (
              <div className="p-2.5 bg-sky-50/70 border border-sky-200 rounded text-xs text-sky-900">
            ...
            {fields.length > 0 ? (
              <>
                {fields.map((field) => {
                  ...
                  const matchedVal = getFieldValue(values, field);
                  const value =
                  ...
                  const handleFieldChange = (newVal: unknown) => {
                    setValues((prev) => ({
                      ...prev,
                      [fieldKey]: newVal,
                      ...(field.id && field.id !== fieldKey ? { [field.id]: newVal } : {}),
                    }));
                  };
                    ...
                    <div key={field.id} className="space-y-1">
                      ...
                      {field.type === "singleChoice" || (options && options.length > 0) ? (
                        <select
                          value={displayValue}
                          onChange={(e) =>
                            handleFieldChange(e.target.value || null)
                          }
                          className="w-full text-xs px-2.5 py-1.5 border border-slate-300 rounded bg-white focus:outline-none focus:ring-1 focus:ring-sky-500 font-medium text-slate-800"
                        >
                ...
                {/* Extra attributes that may exist on record but not explicit in schema */}
                {(() => {
                  const covered = new Set(
                    fields.flatMap((f) => [
                      f.id,
                      (f as any).key,
                      f.id?.replace(/^f-[a-z0-9]+-/, ""),
                      f.id?.replace(/^f-/, ""),
                    ]).filter(Boolean),
                  );
                  const extra = Object.entries(values).filter(
                    ([k]) => !covered.has(k) && !k.startsWith("f-"),
                  );
                  if (extra.length === 0) return null;
                  return (
                    <div className="pt-3 border-t border-slate-200 space-y-2">
                      <p className="text-xs font-semibold text-slate-700">
                        Otros Atributos Registrados
                      </p>
                        ...
                        {extra.map(([k, v]) => (
                          <div key={k} className="space-y-1">
                            ...
                            <input
                              ...
                              className="w-full text-xs px-2.5 py-1.5 border border-slate-300 rounded bg-white text-slate-800 font-medium"
                            />
                          </div>
                        ))}
                      </div>
              ...
              <div className="space-y-3">
                <p className="text-xs text-slate-500 italic">
                  Mostrando todos los atributos del registro:
                </p>
